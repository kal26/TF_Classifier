# From Avanti Shrikumar / DEEPLIFT
import matplotlib
...
def plot_icweights(array, figsize=(20,2),
                 height_padding_factor=0.2,
                 length_padding=1.0,
                 subticks_frequency=25,
                 colors=default_colors,
                 plot_funcs=default_plot_funcs,
                 highlight={}):
    weighted_array = np.zeros(array.shape)
    weighted_array = np.asarray([information(base) * base for base in array])
    plot_weights(weighted_array, figsize=figsize,
                 height_padding_factor=height_padding_factor,
                 length_padding=length_padding,
                 subticks_frequency=subticks_frequency,
                 colors=colors,
...
def information(distribution):
    ...
    return max_ic + sum([x * math.log(x+1e-8, 2) for x in distribution]) 
