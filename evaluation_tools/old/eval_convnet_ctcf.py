...
class ctcfmodel(object):
    ...
    def __init__(self, outpath, gen_path='/home/kal/data/ctcfgen_data.hdf5', genome_path='/home/kal/.ucscgenome/hg19.2bit', bed_path=None, batch_size=32):
        ...
        self.gen = ctcfgen.CTCFGeneratorhdf5(gen_path)
    ...
    def get_pr(self, show=False, mode='atac', const=False, gain=False):
        if mode == 'atac':
            return self.get_atac_pr(show=show, const=const, gain=gain)
        elif mode == 'test':
            return self.get_test_pr(show=show, gain=gain)

    def get_test_pr(self, show=False, gain=False):
        batch_size = self.batch_size
        g = self.gen.pairgen(mode='test')
        # Get predicitons.
        preds = self.model.predict_generator(g, self.gen.test_pos.shape[0] // batch_size).flatten()
    ...
    def get_atac_pr(self, const=False, show=False, gain=False):
        ...
        if const:
            deep_precision, deep_recall, thresholds = precision_recall_curve(self.constrained_peaks['ctcf'].tolist(),
                                                                 expit(self.constrained_peaks['ml'].tolist()), pos_label=1)
            ...
            deep_precision, deep_recall, thresholds = precision_recall_curve(self.peaks['ctcf'].tolist(), 
                                                                 expit(self.peaks['ml'].tolist()), pos_label=1)
    ...
    def get_tsne(self, layer_name, show=False):
        ...
        g = ctcfgen.filled_batch(iter(self.sample_peaks['signal_seq']), batch_size=self.batch_size)
    ...
    def varience_plot(self, layer_name, show=False):
        ...
        ml = np.repeat(np.expand_dims(np.clip(self.sample_peaks['ml'], None, 50), axis=1), 2, axis=1)
        pwm = np.repeat(np.expand_dims(self.sample_peaks['pwm'], axis=1), 2, axis=1)
        diff = np.repeat(np.expand_dims(self.sample_peaks['pwm'] - np.clip(self.sample_peaks['ml'], None, 50), axis=1), 2, axis=1)
    ...
    def generate_bed(self, split_atac_path='/home/kal/data/intermediates/split_atac_peaks.bed', full_atac_path='/home/kal/data/intermediates/full_atac_peaks_ctcf_pwm.bed'):
        """ generate and annotate a bed file of full atac peaks.
            
            Keywords:
                split_atac_path -- path to a bed file at atac peaks split into overlapping 256 bp windows.
                full_atac_path -- path to a bed file of full atac peaks with ctcf and pwm annotations
        """
        # get a dataframe of the bed file
        peaks = pd.read_table(split_atac_path, header=None)
        peaks.columns = 'chr start end'.split() 

        # get predictions for the bed file
        preds = predict_peaks(self.model, self.genome, peaks)
