...
class BasicBlock(object):
    ...
    def __init__(self, filters, kernel_width=3, decay=0.9999, drop_rate=0.1):
        ...
        self.conv = Conv1D(filters, kernel_width, padding='same', activation=None)#Add leaky relu?
        # weight decay
...
class BasicRes(object):
    """Object for a series of convolutions add residual connection."""
    def __init__(self, num_inputs, conv_list, drop_rate=0.0, res=True):
        """Initialize a BasicRes object.
        ...
        self.res = res
        if self.res and (num_inputs[1] != conv_list[-1][0]):
            print('fixing convolution to be residual')
            conv_list.append([num_inputs[1], 3])

        blockgen = BasicBlock
        print('Convolutions used: ' + str(conv_list) + ' [neurons, filter]')
        self.conv_in = Conv1D(conv_list[0][0], conv_list[0][1], padding='same', activation=None, input_shape=(num_inputs))
