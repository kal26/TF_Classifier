...
def main(argv=None):
    ...
    if FLAGS.gen_path != None:
        ...
        if '.hdf5' in FLAGS.gen_path:
            gen = ctcf_strength_gen.CTCFGeneratorhdf5(FLAGS.gen_path)
        ...
        try:
            gen = ctcf_strength_gen.CTCFGenerator()
        except OSError:
    ...
    model.compile(loss='mean_squared_error', optimizer=optimizer_1)
    ...
    if FLAGS.finer_epochs:
        ...
        checkpointer = ModelCheckpoint(checkpath, verbose=1, monitor='val_loss', mode='min')
        callbacks_list = [checkpointer]
        model.compile(loss='mean_squared_error', optimizer=optimizer_2)
        ...
        checkpointer = ModelCheckpoint(checkpath, verbose=1, monitor='val_loss', mode='min')
        callbacks_list = [checkpointer]
        model.compile(loss='mean_squared_error', optimizer=optimizer_3)
